...
// ^^^^ See above for problem statement.
...
void main() {
  List inputs;
  int surfaceN = int.parse(stdin
      .readLineSync()); // the number of points used to draw the surface of Mars.
  for (int i = 0; i < surfaceN; i++) {
    inputs = stdin.readLineSync().split(' ');
    int landX =
        int.parse(inputs[0]); // X coordinate of a surface point. (0 to 6999)
    int landY = int.parse(inputs[
